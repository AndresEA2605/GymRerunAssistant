...
export default function LevelUpOverlay() {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none p-4">
      ...
      <div className="relative animate-in zoom-in-50 fade-in duration-500 slide-in-from-bottom-10 flex flex-col items-center justify-center drop-shadow-[0_0_80px_rgba(99,102,241,0.6)]">
        {/* Glow effect */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-indigo-500/30 blur-3xl rounded-full" />
        ...
        <h1 className="relative z-10 text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-indigo-200 tracking-tight text-center drop-shadow-xl mb-2">
          ¡NIVEL {showLevel}!
        </h1>
        
        <p className="relative z-10 text-lg md:text-xl font-bold text-indigo-300 mb-4 tracking-wide text-center uppercase">
          {milestone?.title ? `Has alcanzado: ${milestone.title}` : '¡Sigue así!'}
        </p>

        {(milestone?.bonusXP || milestone?.bonusCoins) && (
          <div className="relative z-10 flex items-center gap-4 bg-neutral-900/80 border border-indigo-500/30 px-6 py-3 rounded-2xl">
            {milestone.bonusXP && (
