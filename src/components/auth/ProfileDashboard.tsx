...
import { X, LogOut, Zap, Flame, Trophy, Star, Flag, Clock, Route, Swords, Users, Footprints, Target, MapPin, Lock, Check, ChevronDown } from "lucide-react";
...
export default function ProfileDashboard({ isOpen, onClose }: ProfileDashboardProps) {
  ...
  const { profile, stats, setActiveTitle, manager } = useProgression();
  ...
  const unlockedAchievementIds = manager?.unlockedAchievements ?? [];
  const unlockedTitleIds = manager?.unlockedTitles ?? [];
  ...
  return createPortal(
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
      ...
      <div
        className="relative z-10 w-full max-w-lg max-h-[85vh] overflow-y-auto bg-gradient-to-b from-neutral-900 via-neutral-900 to-neutral-950 rounded-3xl border border-neutral-700/40 shadow-[0_0_80px_rgba(99,102,241,0.06)] animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 scrollbar-thin scrollbar-thumb-neutral-800"
        onClick={e => e.stopPropagation()}
      >
        <button onClick={onClose} className="fixed top-4 right-4 z-20 w-9 h-9 rounded-full bg-neutral-800/60 hover:bg-neutral-700 flex items-center justify-center text-neutral-400 hover:text-white transition-all">
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="flex items-center gap-4 px-6 pb-4">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-sm font-bold text-violet-300">{profile.activityPoints}</span>
            <span className="text-[10px] text-neutral-600 font-bold">P. Actividad</span>
          </div>
          <div className="flex items-center gap-1.5 ml-auto">
            <Flame className="w-4 h-4 text-orange-400" />
            <span className="text-sm font-bold text-orange-300">{profile.currentStreak}</span>
        ...
        <div className="px-6 pb-4 flex items-center gap-3">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-orange-950/20 border border-orange-800/20">
            <Flame className="w-4 h-4 text-orange-400" />
            <span className="text-xs font-bold text-orange-300">Actual: {profile.currentStreak}d</span>
          </div>
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-950/20 border border-amber-800/20">
            <Star className="w-4 h-4 text-amber-400" />
            <span className="text-xs font-bold text-amber-300">Mejor: {profile.bestStreak}d</span>
          </div>
