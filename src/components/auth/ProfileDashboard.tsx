...
export default function ProfileDashboard({ isOpen, onClose }: ProfileDashboardProps) {
  ...
  const { profile, isLoaded } = useProgression();
  ...
  const xpPct = profile ? xpProgressInLevel(profile.totalXP) : 0;
  const xpCurrent = profile ? profile.totalXP - xpForLevel(profile.level) : 0;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
      ...
      <div
        className="relative w-full max-w-lg max-h-[85vh] overflow-y-auto bg-gradient-to-b from-neutral-900 via-neutral-900 to-neutral-950 rounded-3xl border border-neutral-700/40 shadow-[0_0_80px_rgba(99,102,241,0.06)] animate-in zoom-in-95 slide-in-from-bottom-4 duration-300"
          ...
          <div className="px-6 pb-6 animate-in fade-in duration-300">
                ...
                {profile && (
                  <div className="flex items-center gap-2 mt-1.5">
                    <div className="flex-1 h-1.5 rounded-full bg-neutral-800 overflow-hidden">
                      <div
                        className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-violet-500 progress-shimmer animate-xp-fill"
                        style={{ width: `${Math.min(100, xpPct)}%` }}
                      />
                    </div>
                    <span className="text-[10px] font-bold text-neutral-400 shrink-0">
                      {xpCurrent}/{profile.xpToNextLevel} XP
                    </span>
                  </div>
                )}
              </div>
            ...
            {stats ? (
              <>
                ...
                <div className="mt-6">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      ...
                      { icon: <Trophy className="w-4 h-4 text-amber-400" />, label: "Gyms", value: stats.totalGyms },
                    ].map(s => (
                      <div key={s.label} className="flex flex-col items-center gap-1 px-1 py-2.5 rounded-xl bg-neutral-800/40 border border-neutral-800/30 hover:border-neutral-700/50 transition-colors">
                        {s.icon}
                ...
                <div className="mt-4 flex items-center gap-3">
                  ...
                  {profile && (
                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-950/20 border border-indigo-800/20 ml-auto">
                      <Zap className="w-3.5 h-3.5 text-indigo-400" />
                      <span className="text-xs font-bold text-indigo-300">Nv. {profile.level}</span>
                    </div>
                  )}
                </div>
              </>
            ) : (
              <div className="mt-6 text-center text-neutral-500 text-sm">Cargando estadísticas...</div>
            )}
            ...
            <div className="mt-6 pt-4 border-t border-neutral-800/40">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-2xl text-red-400 hover:text-red-300 hover:bg-red-950/30 border border-red-900/30 hover:border-red-800/50 transition-all text-sm font-bold active:scale-[0.98]"
              >
