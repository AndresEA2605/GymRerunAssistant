...
function getTransporterConfig() {
  ...
  const port = Number(process.env.SMTP_PORT || process.env.GMAIL_PORT || 465);
  const secure = (process.env.SMTP_SECURE || process.env.GMAIL_SECURE || 'true').toLowerCase() === 'true';
...
export async function sendPasswordResetEmail(email: string, token: string) {
  ...
  const mailOptions = {
    from: process.env.SMTP_FROM || process.env.GMAIL_FROM || (sender ? `"PokeAssistant" <${sender}>` : '"PokeAssistant" <noreply@pokeassistant.com>'),
    to: email,
